...
class LoggedMiddleware extends BasicMiddleware {
	...
	static loggedCallback(request, response, next, middleware, staticMiddleware) {
		return middleware.constructor.logged(new ParametersManager({ middleware: middleware, staticMiddleware: middleware.constructor }, request, response, next, {}));
	}
	...
	static async logged(_) {
		try {
			...
			console.log(error);
			return _.response.sendJsonError(StringUtils.stringify(error), {}, 500);
		}
	}
	...
	static onGetBearerToken(_) {
		...
		if(!_.storage.bearerToken) {
			throw new ErrorManager.classes.AuthenticationError("UnauthenticatedError", "Could not find bearer token in http headers.");
		}
	...
	static onPrepareFields(_) {
		...
		_.storage.sessionQuery.field("s.secret_token", "session secret token");
		_.storage.sessionQuery.field("s.recovery_token", "session recovery token");
	...
	static onAuthenticateToken(_) {
		if(!_.storage.sessionResult.length) {
			throw new ErrorManager.classes.AuthenticationError("UnauthenticatedError", "Could not find the provided secret token as active session.");
		}
	}
	...
	static onGroupResults(_) {
		...
		_.request.expolium.auth = new AuthenticationManager(_.storage.authData);
	}
